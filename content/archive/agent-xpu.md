...
记几条我自己没想透、之后想接着看的方向，未必都对：

- **Bcap=3 是不是普适的**。这是在 Intel Ultra 5 + Llama-3B/8B 上 profiling 出来的，换硬件（比如手机的高通 Hexagon NPU）、换模型大小，最优值肯定不一样。有没有办法做成自适应的——根据当前 KV cache 总量和带宽利用率动态调？
- **NPU AOT 内核库的覆盖率**。论文用 chunk_size=256 + 后台预热兜余量，但 chunk 大小本身是个超参数。chunk 太小 NPU 利用率不够，chunk 太大不能整除剩的余量更多，更容易触发预热失败的 fallback。这个 tradeoff 论文里说了多少？有没有更聪明的多 chunk size 组合？
- **和 KVFlow 的关系**。KVFlow 解决的是多 agent workflow 里 KV cache 跨调用复用的调度感知，Agent.xpu 解决的是 SoC 上异构计算单元的协同。两者其实是正交的——一个管「KV cache 怎么留」，一个管「计算往哪个核扔」。如果在 SoC 上跑多 agent workflow，两个机制叠加会怎样？KVFlow 的预取要走 PCIe，SoC 上是统一内存就不存在这个开销，但调度感知的淘汰策略仍然有效。
- **能耗 vs 延迟的真实曲线**。论文主要报了延迟和吞吐，但 SoC 上能耗其实是更硬的约束（电池就那么大）。NPU 比 iGPU 能效高很多倍，所以「尽量用 NPU」不只是为了快，也是为了省电。混合调度对总能耗的影响，论文有没有给具体数字？
- **stream-aware 抽象能不能做成通用 runtime 接口**。现在 OpenVINO、SGLang 这种引擎本质上是单流的，agent 要自己管多任务。如果把「reactive / proactive」这种 stream 标签提升到推理引擎的标准接口，做一个统一的 stream-aware runtime，是不是能让上层 agent 框架（Magentic-One、AutoGen 这种）直接吃到这个收益？感觉这是一个值得做的工程方向。
